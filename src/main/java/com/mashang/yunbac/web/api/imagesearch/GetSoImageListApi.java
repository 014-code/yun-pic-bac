...
public class GetSoImageListApi {
     ...
     * @return 图片列表对象
     */
    public static List<ImageSearchVo> getImageList(String imageUrl, Integer start) {
        ...
        List<ImageSearchVo> result = data.getBeanList("result", ImageSearchVo.class);
        // 对结果进行处理, 因为返回的是分开的对象, 不是一个完整的图片路径, 这里需要自己拼接
        for (ImageSearchVo soImageSearchResult : result) {
            String prefix;
